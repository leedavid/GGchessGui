...
HandleMachineCYC105(char *message, ChessProgramState *cps){
    ...
    //LogOut(ColorRequest,message,true);
        ...
        if(strlen(message) < 2){
            ...
            switch(pDis->gameMode){
                case BeginningOfGame:
                    ...
                    //    fprintf(DIS.debugFP, "Ignoring extra move from %s\n", cps->UCCI_name);
                    //}
#endif
                    return;                    
                case EndOfGame:
            ...
            if(!ParseOneMove(pmove,pDis->pos->gply,&moveType,&fromX, &fromY, &toX, &toY, cps->ucci)) {
                ...
                StringCbPrintf(buf,MSG_SIZ,"Illegal move \"%s\" from %s machine",message, cps->UCCI_name);
                DisplayError(buf, 0);
            ...
            if(pDis->gameMode == TwoMachinesPlay){    //是双引擎对战
                ...
                SendProgramGo(cps->other);                              //使用引擎开始运算
            ...
            cps->maybeThinking = FALSE;     //已返回走步,就不可能再思考了
    ...
    //*******************************************************************************
        ...
        if(false){
            ...
            if(p != 0){
                p += 8;
                if(strncasecmp(p,"true",4) == 0){
                    cps->UCCI_usebook = true;
                }
                else{
                    cps->UCCI_usebook = false;
                }
                if(cps->UCCI_usebook){
                    LogOut("引擎自带开局库\n");
                }
    ...
    //*******************************************************************************
    else if(strncasecmp(message,"cciok", 5) == 0){

        if(cps->UCCI_initDone == FALSE){

            cps->UCCI_initDone = TRUE; 
            cps->yqOldId       = cps->yqID;             //保存上一次的引擎ID
            SetCpsPlayWhat(cps);
            ...
            //1, 得到CPS的控制参数
            GetTimeSetInfoFromCps(cps);

            //
            //if(cps->other->playWhat == NEUTRAL){
            //    GetTimeSetInfoFromCps(
            //}
        ...
        //}
        //else{
        //    SendMessage(hwndToolBar,TB_ENABLEBUTTON,IDT_COM_BLUE,1);
        //}
    }
    //*******************************************************************************
